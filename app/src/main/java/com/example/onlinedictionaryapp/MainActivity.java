...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            rs=ps.executeQuery();//to select dictionary word to store in rs variable
            while(rs.next())//loop is execute until result set having data
            {
              al.add(rs.getString("word"));
            }
            Ad=new ArrayAdapter(this, android.R.layout.simple_dropdown_item_1line,al);
            actv_word.setAdapter(Ad);//to show all list item
